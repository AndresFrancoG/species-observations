...
def test_preprocessing_class_attributes(kedro_env: str, catalog_entry: str):
    ...
    name_types = utl.attribute_names_types(
        parameters[catalog_entry]["tests"]["member_variables"], prep, type_mapping
    )
...
def test_time_resampling_index(kedro_env: str, catalog_entry: str):
    ...
    df_sample = utl.load_csv_from_catalog(
        kedro_env,
        config_entry=catalog_entry,
        entry_name="csv_sample_catalog_preprocessed_stage_01",
    )
...
@pytest.mark.parametrize(
    ...
    [("test_cloud", "preprocessing", "D"), ("test_cloud", "preprocessing", "M")],
)
def test_time_resampling_diff(kedro_env: str, catalog_entry: str, resample: str):
    ...
    if resample == "D":
        min_time_diff = df_out.index.to_series().diff().min().total_seconds() / (
            3600 * 24
        )
    elif resample == "M":
...
@pytest.mark.parametrize(
    ("kedro_env", "catalog_entry", "resample"),
    [("test_cloud", "preprocessing", "X"), ("test_cloud", "preprocessing", "Y")],
)
